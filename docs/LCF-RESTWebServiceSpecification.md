...
NOTE – *(added in vx.x.0)* This REST web service implementation of the LCF Data Framework does not implement a mechanism for specifying the type of a renewal, which in the Data Framework is represented by element Q11D02. In the Data Framework this element serves two purposes: (a) to indicate if the request is to renew all items checked out to that patron; (b) to indicate whether the request is being made by the patron directly or by a third party. In a REST web service implementation it is not possible to request the renewal of all items in a single request, as the loan must be renewed for each item explicitly and separately. In a REST web service implementation it is not necessary to have a separate element or parameter to indicate whether it is the patron or a third party making the request, because this is already indicated by ensuring that the user making the request is authenticated in the request header.
